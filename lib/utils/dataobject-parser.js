...
// True if instance of Immutable.js
...
DataObjectParser.untranspose = function($structured) {
  ...
  var traverse = function($data, $isIndex) {
    ...
    for(var name in $data) {
      ...
      if(_isString($data[name]) || _isNumber($data[name]) || $data[name]===null || _isBoolean($data[name]) || _isDate($data[name]) || _isImmutable($data[name])) {
